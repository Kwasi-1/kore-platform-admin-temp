...
import { HelpCircle, Eye, EyeOff, Check } from 'lucide-react';
...
// Main platform plans available for tenant provisioning
const AVAILABLE_PLANS = ['starter', 'standard', 'business', 'ecom_only'].map(
  (key) => PLANS_CONFIG[key]
);
              ...
              <Form className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="border-b border-border pb-2">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground font-header">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {AVAILABLE_PLANS.map((plan) => {
                        const isSelected = values.plan === plan.key;
                        return (
                          <button
                            key={plan.key}
                            type="button"
                            onClick={() => setFieldValue('plan', plan.key)}
                            className={clsx(
                              'relative flex flex-col justify-between gap-2.5 rounded-xl p-4 border text-left transition-all duration-150',
                              isSelected 
                                ? 'border-primary bg-primary/5 ring1 ring-primary/20' 
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className="text-[11px] font-medium text-foreground bg-secondary px-2.5 py-0.5 rounded-full border border-border">
                                {formatGHS(plan.priceMonthly)}/mo
                              </span>
                            </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Field
                        ...
                        readOnly={true}
                        className="rounded-xl h-10 pr-10 bg-muted/40 cursor-default border-border focus:ring-0 focus:border-border"
                      />
                      <button
                        type="button"
