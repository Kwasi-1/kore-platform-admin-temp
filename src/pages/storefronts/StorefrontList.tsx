    ...
    <PageLayout
      ...
      actions={
        <Button
          ...
          className="bg-primary text-primary-foreground hover:bg-primary/95 rounded-xl font-bold text-xs h-10 px-4 flex items-center gap-2 shadow-sm"
        >
    ...
    >
      <div className="space-y-6">
        ...
        {pendingRequests.length > 0 && (
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Merchants on Business & Ecom plans ready for storefront deployment: {pendingRequests.map(t => t.business_name).join(', ')}.
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              className="bg-amber-500 text-white hover:bg-amber-600 rounded-lg text-xs font-bold px-3 h-8 shrink-0"
            >
              Deploy for {pendingRequests[0].business_name}
            </Button>
          </div>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <DashboardCard
            ...
            action={<Store className="h-5 w-5 text-primary" />}
          />
          <DashboardCard
        ...
        {isLoading ? (
          ...
          <div className="bg-card border border-border rounded-xl p-12 text-center space-y-3 shadow-xs">
            ...
            <Button
              onClick={() => handleOpenProvision()}
              variant="outline"
              className="rounded-xl font-bold text-xs h-9 px-4 mt-2"
            >
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {storefronts.map((store) => {
              ...
              return (
                <div
                  key={store.id}
                  className="bg-card border border-border rounded-xl p-5 space-y-4 shadow-sm flex flex-col justify-between hover:border-muted-foreground/30 transition-all group"
                >
                  <div className="space-y-3">
                    {/* Header: Store Name & Plan */}
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="text-sm font-bold text-foreground font-header group-hover:text-primary transition-colors">
                          {store.tenant_name}
                        </h4>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      {store.custom_domain ? (
                        <span className="inline-flex items-center gap-1 text-[10px] bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded-full font-medium">
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-muted/20 border border-border/50 rounded-lg p-2.5 text-xs">
                      <div>
                  ...
                  <div className="pt-3 border-t border-border flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <Button
                        ...
                        className="rounded-lg h-8 text-[11px] font-semibold px-2.5"
                      >
                        <Power className={clsx("h-3.5 w-3.5 mr-1", isLive ? "text-emerald-500" : "text-amber-500")} />
